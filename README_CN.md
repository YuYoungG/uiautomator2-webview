...
<p align="center">
  <a href="./README.md">English</a> · <a href="./README_CN.md"><strong>简体中文</strong></a>
</p>
...
在进行“Android 原生 + WebView H5”的混合应用自动化测试时，开发者通常会面临以下割裂感：

- **`chrome://inspect/#devices` 类型工具的局限**：它很适合前端手动调试，但无法直接编写自动化脚本，也不能操作 Android 原生控件。
- **`uiautomator2` 的局限**：它擅长操控 Android 原生 UI，但面对嵌套在 App 内部的 WebView 页面时，难以精准提取和交互 HTML 元素。

**`u2_webview` 用来解决二者的局限。**
它让你在同一个 Python 自动化脚本中，既能用 `u2` 流畅操作 Android 原生外壳，又能随时调用 `u2_webview` 穿透进入 H5 内部，真正实现“Android 原生 + WebView”混合应用的无缝自动化协同。
...
- **免驱动接管（Driverless）**：不同于传统的 Selenium / Appium，本库无需下载、配置或匹配特定版本的 `chromedriver`。它通过 CDP 协议直接与 WebView 通信，彻底告别驱动版本不匹配的问题。
- **Flask 扩展模式设计**：遵循 Flask 插件设计哲学，支持“应用工厂”模式，实现与 `uiautomator2.Device` 实例的完全解耦。
- **高性能通信**：基于 `adbutils` 建立高效的端口转发隧道，确保 H5 操作拥有高响应速度与稳定性。
...
3. 为“探索测试”而生：传统工具基于线性脚本思维，遇到异常容易直接崩溃。本库配合自带的 `@with_webview` 装饰器，契合非线性、高频次的探索测试，如与 Kea2 工具结合使用。
...
Kea2 仓库地址：https://github.com/ecnusse/Kea2
