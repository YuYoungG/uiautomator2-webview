...
class Webview:
    ...
    def _get_active_sockets(self):
        ...
        if output:
            ...
            for line in lines:
                match = re.search(r"webview_devtools_remote_\d+", line) or re.search(
                    r"chrome_devtools_remote_\d+", line
                )
                if match:
                    sock = match.group(0)
    ...
    def detach(self):
        ...
        if self.d and self.local_port:
            try:
                adb.device(serial=self.d.serial).forward_remove(
